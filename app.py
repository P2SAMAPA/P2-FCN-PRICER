...
def get_market_data(tickers, tenor_mo, rf_choice, vol_mode, vol_window):
    ...
    for ticker in ticker_list:
        try:
            ...
            prices[ticker] = hist
            if vol_mode == "Real-time Implied": ivs.append(0.32) 
            else:
        ...
        with f_c2:
            ...
            for i, ko in enumerate(BARRIERS):
                for j, sk in enumerate(STRIKES):
                    l_val, ls, y = eng.run_simulation(sk, ko, n_sims=300)
...
with tab2:
    ...
    if run_bcn:
        with bc2:
            ...
            final_c = active_corr if corr_mode == "Manual Slider" else (h_c if "Historical" in corr_mode else min(1.0, h_c + 0.2))
            eng_b = PricingEngine(v, rf, b_te, b_fr, b_nc, b_ks, b_sd, "BCN", correlation=final_c, gtd_rate=b_gtd, bonus_rate=b_bon, bonus_barr=b_bar)
            ...
            for i, ko in enumerate(BARRIERS):
                for j, sk in enumerate(STRIKES):
                    l_val, ls, y = eng_b.run_simulation(sk, ko, n_sims=300)
                    y_m[i,j], l_m[i,j] = y, ls
                    p.progress((i*5+j+1)/25)
            df_y, df_l = pd.DataFrame(y_m, BARRIERS, STRIKES), pd.DataFrame(l_m, BARRIERS, STRIKES)
            cc, cd = st.columns(2)
            cc.write("**Yield Matrix**"); cc.dataframe(df_y.style.background_gradient(cmap="RdYlGn").format("{:.2f}%"), use_container_width=True)
            cd.write("**Loss Matrix**"); cd.dataframe(df_l.style.background_gradient(cmap="YlOrRd").format("{:.2%}"), use_container_width=True)
            st.download_button("📥 Download", create_excel(df_y, df_l) if b_fmt=="Excel" else create_pdf("BCN", df_y, df_l), f"BCN_Report.{b_fmt.lower()}", mime="application/pdf" if b_fmt=="PDF" else "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
