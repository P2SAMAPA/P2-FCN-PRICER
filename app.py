...
with tab2:
    ...
    with bc1:
        ...
        b_gtd = st.number_input("Guaranteed Coupon (%)", 2.0, key="bgtd")
        b_bon = st.number_input("Bonus Coupon (%)", 8.0, key="bbon")
    ...
    if run_bcn:
        with bc2:
            ...
            eng_b = PricingEngine(v_list_b, rf_b, b_te, b_fr, b_nc, b_ks, b_sd, "BCN", gtd_rate=b_gtd, bonus_rate=b_bon, bonus_barr=b_bar)
            ...
            c_c.write("**Yield Matrix (KO vs Strike)**")
            c_c.dataframe(pd.DataFrame(y_m_b, BARRIERS, STRIKES).style.background_gradient(cmap="RdYlGn").format("{:.2f}%"), use_container_width=True)
            c_d.write("**Loss Matrix (KO vs Strike)**")
            c_d.dataframe(pd.DataFrame(l_m_b, BARRIERS, STRIKES).style.background_gradient(cmap="YlOrRd").format("{:.2%}"), use_container_width=True)
