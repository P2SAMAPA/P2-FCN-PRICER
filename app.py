...
def get_market_data(tickers):
    ...
    for t in tickers:
        ...
        try:
            ...
            iv = hist['Close'].pct_change().std() * np.sqrt(252) 
        vols.append(iv)
...
def price_fcn(coupon_pa, paths, r, tenor, strike, ko, freq_m, nc_m):
    ...
    for d in obs_dates:
        ...
        # KO only possible AFTER the non-call period
        if d >= nc_steps:
...
with st.sidebar:
    ...
    tk_in = st.text_input("Underlying Tickers", "SLV, GLD")
    tickers = [x.strip().upper() for x in tk_in.split(",")]
    vol_source = st.radio("Volatility Input", ["Market Implied", "Historical"])
    ...
    nc_m = st.number_input("Non-Call Period (Months)", 0, 12, 3) # RESTORED
...
if st.button("Generate FCN Valuation"):
    ...
    paths = run_simulation(10000, tenor_y, rf_rate, vols, corr, skew_val)
    
    # Solver
    cpn_rng = np.linspace(0.0, 0.80, 40)
    ...
    st.write("#### Expected Coupon Payout Probability")
    dist = pd.Series(res['cpn_dist']).value_counts(normalize=True).sort_index() * 100
    st.plotly_chart(px.bar(x=dist.index, y=dist.values, 
                           labels={'x': f'Number of {freq_m}-Month Coupons', 'y': 'Probability (%)'},
                           title="Distribution of Total Coupons Received before KO or Maturity"))
