...
# --- 1. RESTORE PROFESSIONAL UX STYLING (Matches image_e2515c) ---
...
# --- 2. RESTORE FULL SIDEBAR UX (Matches image_e25b9e) ---
with st.sidebar:
...
# --- 3. RECTIFIED PRICING ENGINE (Fixes 67% Yield Error) ---
@st.cache_data(ttl=3600)
...
# --- 4. RESTORED MAIN CONTENT UX (Matches image_e2515c) ---
if len(tks) >= 1:
    ...
    if names:
        ...
        # UI RESTORATION: Product Configuration
        ...
        # Bonus inputs for BCN Solver
        b_rate, b_ref = 0, 0
        if "BCN" in mode:
            c1, c2 = st.columns(2)
            b_rate = c1.number_input("Bonus Rate %", 0.0, 40.0, 8.0)/100
            b_ref = c2.number_input("Bonus Strike %", 70.0, 110.0, 100.0)

        if st.button("Generate Pricing Report"):
            sol = brentq(lambda x: run_pricing_logic(x, ps, rf_rate, tenor_y, stk_val, ko_val, fq, nc_m, mode, sd=sd, b_r=b_rate, b_f=b_ref)[0] - 100, 0, 1.0)
            val, prob_loss = run_pricing_logic(sol, ps, rf_rate, tenor_y, stk_val, ko_val, fq, nc_m, mode, sd=sd, b_r=b_rate, b_f=b_ref)
            
            # UI RESTORATION: Three-Column Metric Cards
